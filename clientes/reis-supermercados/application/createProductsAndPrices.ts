...
const processProductBatch = async (
  ...
  try {
    ...
    console.log("[REIS-SUPERMERCADOS] :: ",err);
...
async function createProductsAndPrices() {
  try {
    ...
    console.log(`[REIS-SUPERMERCADOS] :: Script finalizado com sucesso! -- ${time} segundos `)
    return `[REIS-SUPERMERCADOS] :: Script finalizado com sucesso! -- ${time} segundos `
  } catch (error) {
    throw new Error(`[REIS-SUPERMERCADOS] :: Erro ao executar script ${error}`);
  }
}
