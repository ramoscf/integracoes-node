...
const processProductBatch = async (
  ...
  try {
    ...
    console.log("[REVIEWER-SUPERMERCADOS] :: ",err);
...
const processBatch = async (branchId: number): Promise<any> => {
  ...
  for await (const products of apiGateway.getAlteredProducts(branchId)) {
...
async function createProductsAndPrices() {
  try {
    ...
    Log.setClientName('reviewer-supermercados');
    await Promise.all([
    ...
    console.log(`[REVIEWER-SUPERMERCADOS] :: Script finalizado com sucesso! -- ${time} segundos `)

    return `[REVIEWER-SUPERMERCADOS] :: Script finalizado com sucesso! -- ${time} segundos `

  } catch (error) {
    throw new Error(`[REVIEWER-SUPERMERCADOS] :: Erro ao executar script ${error}`);
  }
}
