...
import { PriceDTO } from '../domain/dtos/PriceDTO';
...
const transformRecord = (
  ...
  return Price.createRegularPrice({
...
const fetchDataStreamFromDatabase = async (queryRunner: QueryRunner): Promise<Readable> => {
  ...
  return queryRunner.stream(`
    ...
    PRECOREGULAR,
    PRECOPROMOCIONAL
    FROM CLT158356CARTAZFACIL.CTZFCL_PRECOS
    WHERE TRUNC(DTAVALIDACAOPRECO) >= TO_DATE('${today}', 'YYYY-MM-DD')
    `)
...
const processBatchChunk = async (batch: PriceDTO[]) => {
  const transformedBatch = batch.map(transformRecord);
  const codes = [...new Set(transformedBatch.map((record) => record.getState().prodCod))];
...
async function createPrices() {
  try {
    ...
    //  await createSSHTunnel();
     //
     console.log(`[JB-SUPERMERCADOS] :: Iniciando criação de Preços`);
