...
const processProductBatch = async (
  ...
  try {
    ...
    console.log("[AMERICA-SUPERMERCADOS] :: ",err);
...
async function createProductsAndPrices() {
  try {
    ...
    Log.setClientName('america-supermercados');
    await Promise.all([
    ...
    console.log(`[AMERICA-SUPERMERCADOS] :: Script finalizado com sucesso! -- ${time} segundos `)
    return `[AMERICA-SUPERMERCADOS] :: Script finalizado com sucesso! -- ${time} segundos `
    } catch (error) {
      throw new Error(`[AMERICA-SUPERMERCADOS] :: Erro ao executar script ${error}`);
    }
}
